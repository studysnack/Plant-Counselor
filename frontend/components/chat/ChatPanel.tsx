...
// ── markdown renderer for AI output (dependency-free) ───────
// A small, fully self-contained markdown renderer. We avoid an external library
// because the ESM markdown packages did not render reliably in this Next/Turbopack
// runtime. Supports: # headings, --- hr, - / * / 1. lists, ``` code fences,
// > blockquote, paragraphs with line breaks, and inline **bold** _italic_ `code` [links].

// Inline: bold, italic, inline-code, links. Tried in priority order per position.
const INLINE_RE =
  /(`[^`]+`)|(\*\*[^*]+\*\*)|(__[^_]+__)|(\*[^*\n]+\*)|(_[^_\n]+_)|(\[[^\]]+\]\([^)\s]+\))/;
...
function MarkdownText({ text }: { text: string }) {
  ...
  while (i < lines.length) {
    ...
    // Paragraph — gather consecutive non-blank, non-block lines; single newlines → <br>.
    const para: string[] = [];
