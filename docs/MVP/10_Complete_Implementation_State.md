...
|------|------|
| 통계 차트 | 주간/월간 진행률 그래프 |
| 주/일 캘린더 | 월간 뷰 날짜 드래그는 구현됨. 시간 블록 드래그 편집은 별도 주/일 뷰 설계 필요 |
...
> 승인 흐름, 시간대 충돌 경고, 최근 삭제/상태 변경 되돌리기는 이후 구현됨.
> 식물 상세 봉우리 목록의 상태/진행률 범위/마감일 필터와 최근 수정/마감/진행률/상태 정렬도 이후 구현됨.
> 좁은 화면에서는 좌측 사이드바 대신 하단 탭 내비게이션을 사용하도록 이후 개선됨.
