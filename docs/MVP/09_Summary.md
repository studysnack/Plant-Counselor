...
| 식물 상세 | 통계(활성/수확/포기 카운트) + 봉우리 목록(상태/진행률/마감일 필터, 최근 수정/마감/진행률/상태 정렬) | GET /plants/{id} |
...
| 탭 | 항목 |
|----|------|
| 계정 | Google 프로필 표시, 로그아웃, JSON/CSV/ICS 데이터 내보내기, 계정 삭제 |
...
| 식물 상세 | /plants/[id] | 헤더카드(통계+삭제), 봉우리 목록(상태/진행률/마감일 필터와 정렬), 봉우리 드로어(이력+액션) |
