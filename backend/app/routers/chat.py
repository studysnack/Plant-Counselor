...
Composition is intentionally light: skills & prompt-builder are cached on
module load (cheap, stateless), services & LLM client are constructed per
request because they depend on the request DB session and the authenticated
...
def _build_registry() -> SkillRegistry:
    """ThinkSkill is registered FIRST so the LLM can see the planning tool
    before any data-mutating tools — order influences tool listing in the
    prompt and the LLM's natural tendency to pick the first matching tool."""
    reg = SkillRegistry()
    for skill_cls in (
        ThinkSkill,
        MatchPlantSkill,
...
def _resolve_api_key(db: Session, user: User) -> str:
    """Prefer the user-supplied (encrypted) key, fall back to env-level key."""
    try:
