...
- 로그인 전 랜딩 상단의 `AI 정원사 — ...` 모델 배지는
  `GET /api/v1/public/runtime`의 `llm_default_model_label`을 클라이언트에서 읽어
  표시한다. `/admin/controller`에서 `llm_default_model`을 바꾸면 프리뷰 배지도
  주기적 재조회와 focus 재조회로 같은 값을 따라가야 한다. 공개 API에는 표시 가능한
  런타임 정보만 추가하고 관리자 설정 전체를 노출하지 않는다.
...
- 수확된 봉우리는 식물 화분 디자인을 바꾸지 않고 정원 왼쪽의 별도
  `GardenHarvestBasket` 바구니에 모은다. 바구니와 내부 열매는 Figma `05 에셋`의
  `harvest crate`를 기준으로 하며, 열매에는 줄기나 잎을 붙이지 않는다. 바구니 디자인
  검토용 Figma 프레임은 `00 Cover` 페이지의
  `Harvest basket redesign / 2026-06-03`이다. 화분 대체안으로 되돌리지 않고,
...
- 정원 성장 레이어는 Figma `05 Plant Pixel Assets`의 no-pot extension 좌표를
  ...
  있게 한다. 봉우리 단위 좌우반전은 줄기 중심축을 기준으로 해야 하므로
  `GrowthLayer`의 반전 `transformOrigin`은 줄기 중심인 `90px 0`을 유지한다.
  이를 바꾸면 랜딩 프리뷰와 실제 정원에서 반전된 봉우리 줄기가 1-2px 어긋날 수 있다.
